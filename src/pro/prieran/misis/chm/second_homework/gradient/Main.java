...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) {
        gradient.doIt();
        List<Double> xPoints = gradient.xPoints;
        List<Double> yPoints = gradient.yPoints;
        List<Double> zPoints = gradient.zPoints;
    ...
    private void initXGraph(XYChart.Series<Number, Number> series, int numberOfFunction) {
        ...
        for (double x = FROM; x < TO; x += (TO - FROM) / COUNT_OF_VISIBLE_POINTS) {
            ...
            switch (numberOfFunction) {
                case 1:
                    value = gradient.f1(x, 1.50389, 3.03557);
                    break;
                case 2:
                    value = gradient.f2(x, 1.50389, 3.03557);
                    break;
                case 3:
                    value = gradient.f3(x, 1.50389, 3.03557);
                    break;
                default:
    ...
    private void initYGraph(XYChart.Series<Number, Number> series, int numberOfFunction) {
        ...
        for (double y = FROM; y < TO; y += (TO - FROM) / COUNT_OF_VISIBLE_POINTS) {
            ...
            switch (numberOfFunction) {
                case 1:
                    value = gradient.f1(1.00084, y, 3.03557);
                    break;
                case 2:
                    value = gradient.f2(1.00084, y, 3.03557);
                    break;
                case 3:
                    value = gradient.f3(1.00084, y, 3.03557);
                    break;
                default:
                    throw new IllegalArgumentException();
            }
    ...
    private void initZGraph(XYChart.Series<Number, Number> series, int numberOfFunction) {
        ...
        for (double z = FROM; z < TO; z += (TO - FROM) / COUNT_OF_VISIBLE_POINTS) {
            ...
            switch (numberOfFunction) {
                case 1:
                    value = gradient.f1(1.00084, 1.50389, z);
                    break;
                case 2:
                    value = gradient.f2(1.00084, 1.50389, z);
                    break;
                case 3:
                    value = gradient.f3(1.00084, 1.50389, z);
                    break;
                default:
                    throw new IllegalArgumentException();
            }
